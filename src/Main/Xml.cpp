/*
 ...
 file) and all other portions of this file are Copyright (c) 2013-2025 AM Crypto
...
XmlNodeList XmlParser::GetNodes(const wxString &nodeName) const {
  ...
  while ((nodePos = XmlText.find(L"<" + nodeName, nodePos)) != wxString::npos) {
    ...
    if (nodeTagText.size() > nodeName.size() &&
        nodeTagText[nodeName.size()] != L' ' &&
        nodeTagText[nodeName.size()] != L'/')
      continue;
    ...
    while (attrPos < nodeTagText.length()) {
      ...
      wxString attributeText =
          nodeTagText.substr(quoteStart + 1, quoteEnd - quoteStart - 1);
    ...
    if (!nodeTagText.EndsWith(L"/")) {
      ...
      xmlNode.InnerText = ConvertEscapedChars(
          XmlText.substr(innerTextPos, innerTextEnd - innerTextPos));
...
XmlWriter::XmlWriter(const FilePath &fileName) {
  ...
  *TextOutStream << L"<?xml version=\"1.0\" encoding=\"utf-8\"?>" << endl
                 << L"<01>" << endl;
...
void XmlWriter::Close() {
  if (MemOutStream.get()) {
    *TextOutStream << L"</01>" << endl;

    wxStreamBuffer *buf = MemOutStream->GetOutputStreamBuffer();
    OutFile.Write(
        ConstBufferPtr(reinterpret_cast<uint8 *>(buf->GetBufferStart()),
                       buf->GetBufferSize()));
    OutFile.Close();
  ...
  foreach (const XmlNode &node, xmlNodes) {
    *TextOutStream << indent << L"<" << node.Name;
    ...
    foreach (AttribPair attrib, node.Attributes) {
      *TextOutStream << L" " << attrib.first << L"=\""
                     << EscapeChars(attrib.second) << L"\"";
    }
